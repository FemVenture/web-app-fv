...
export const Button = ({
    ...
    const [variantStyle, setVariantStyle] = useState(variant === "primary" ? "bg-blue-950 text-white" : "bg-white text-blue-950")
    ...
    return (
        <button type={type} onClick={(e) => onClick ? onClick(e) : undefined} disabled={disabled} className={`flex justify-center font-semibold rounded-xl ${borderSizeStyle} ${buttonStyle} ${variantStyle} ${borderColorStyle}`}>
            {label ?? children ?? ""}
        </button>
    )
}